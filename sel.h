...
Matrix createLocalK(int element,mesh &m){
    float D,Ae,E = m.getParameter(THERMAL_CONDUCTIVITY);
    float a = m.getParameter(COM_A);
    Matrix K,A,B,Bt,At;
    ...
    productRealMatrix((-1/2)*(a*E)*Ae/(D*D),productMatrixMatrix(Bt,productMatrixMatrix(At,productMatrixMatrix(A,B,2,2,3),2,2,3),3,2,3),K);
